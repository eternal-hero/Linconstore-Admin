...
export const useUpdatedBalance = (onSuccess: any) => {
  const config = useContext(ContextApi).adminConfig;

  const handleUpdatedBalance =async (data: object) => {
    const response = await axios.post(`${baseUrl}/admin/updatedBalance`, data, config);
    return response.data;
  }
  return useMutation("handleUpdatedBalance", handleUpdatedBalance, { onSuccess });
}

export const useFetchTransaction = (onSuccess: any) => {
  const config = useContext(ContextApi).adminConfig;
  const handleFetchTransactions = async () => {
    const response = await axios.get(`${baseUrl}/admin/transactions`, config);
    return response.data;
  }

  return useQuery("handleFetchTransactions", handleFetchTransactions, {
    onSuccess,
    refetchInterval: 5000,
    enabled: false,
  });
...
export const useFetchVerify = (onSuccess: any) => {
  const config = useContext(ContextApi).adminConfig;
  const handleFetchStores = async () => {
    const response = await axios.get(`${baseUrl}/admin/verification`, config);
    return response.data;
  };
  return useQuery("handleFetchStores", handleFetchStores, {
    onSuccess,
